...
export default function ActivityPickerQueryOptions(context) {
    ...
    if ((!libVal.evalIsEmpty(workOrder)) && (!libVal.evalIsEmpty(operation))) {
        return context.read('/SAPAssetManager/Services/AssetManager.service', `MyWorkOrderOperations(OrderId='${workOrder}',OperationNo='${operation}')`, [], '$expand=WOHeader&$select=WorkCenterInternalId,WOHeader/WorkCenterInternalId,WOHeader/MainWorkCenterPlant,WOHeader/MaintenancePlant,WOHeader/PlanningPlant,WOHeader/CostCenter,WOHeader/ControllingArea').then(function(data) {
            if (data.getItem(0)) {
                let workCenter = '';
